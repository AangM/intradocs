...
- AI default off; hanya WeKnora lokal (`AI_PROVIDER=weknora-local`) pada corpus sintetis. Key tidak dikirim ke chat/browser. Tanpa AI, UI/auth/reader tetap berjalan. Tidak ada provider cloud, auto-billing, atau fallback tanpa persetujuan. Data Telkom nyata memerlukan kebijakan dan endpoint organisasi yang disetujui. Akun demo/seed/`DEMO_LOGIN` mustahil aktif pada profil `production`.
