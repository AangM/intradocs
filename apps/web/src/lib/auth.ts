...
function createAuth(withSso: boolean) {
...
let ssoInstance: ReturnType<typeof createAuth> | undefined;
/**
 * Password sign-in, sign-out and session reads. Deliberately built without the SSO
 * plugin: Better Auth runs the IdP's discovery when a plugin-bearing instance is first
 * used and keeps a failure for the life of the process, which would take password login
 * and every page's session check down with an unreachable IdP. Sessions live in the
 * database, so one created through SSO reads the same here.
 */
export function getAuth() {
  return (instance ??= createAuth(false));
}
/**
 * The SSO endpoints only. The IdP is probed first and the plugin-bearing instance is
 * built only once it answered; until then the caller gets a retryable 503 and nothing
 * else in the portal notices.
 */
