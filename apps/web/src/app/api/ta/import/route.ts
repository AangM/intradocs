...
import { applyTaImport, previewTaImport } from '@intradocs/db/ta';
...
/**
 * Import a Sparx EA export (XMI) or the CSV template into one category.
 * mode=preview parses and compares without writing; mode=apply writes in one
 * transaction. The file must be the same for both: apply carries the preview's hash, so
 * what the admin reviewed is what lands.
 */
export async function POST(request: Request) {
  try {
    ...
    if (mode !== 'preview' && mode !== 'apply') throw new InputError('Mode tidak dikenal.');
    const categoryId = parseUuid(String(form.get('categoryId') ?? ''));
    const bytes = Buffer.from(await file.arrayBuffer());
    const sha256 = createHash('sha256').update(bytes).digest('hex');
    let parsed;
    try {
      parsed = parseTaImport(file.name, bytes.toString('utf8'));
    } catch (e) {
    ...
    const counts = parsed.elements.reduce<Record<string, number>>((acc, e) => {
      acc[e.kind] = (acc[e.kind] ?? 0) + 1;
      return acc;
    }, {});
    if (mode === 'preview') {
      const diff = await previewTaImport(actor.id, categoryId, parsed);
    ...
    const summary = await applyTaImport(actor.id, {
      categoryId,
      parsed,
      filename: file.name,
      format: parsed.format,
    ...
    return NextResponse.json({ ok: true, summary }, { headers: PRIVATE_HEADERS });
  } catch (error) {
