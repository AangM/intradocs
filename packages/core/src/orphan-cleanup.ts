...
const knownFile = new RegExp(
  `^(?:content\\.md|provenance\\.json|(?:original|attachment-[1-4])\\.(?:md|txt|pdf|docx|xlsx|pptx)|\\.tmp-${id})$`,
);
...
async function inspect(root: string, key: string): Promise<OrphanCandidate> {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (
      ...
      s.isSymbolicLink() ||
      s.size >
        (/^(?:original|attachment-[1-4])\.(md|txt|pdf|docx|xlsx|pptx)$/.test(entry.name) ? 50 : 2) *
          1024 *
          1024
    )
