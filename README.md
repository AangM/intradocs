...
2. **AI Assistant** sebagai siti: ketik "Apakah MFA wajib untuk VPN lab?" di kotak **Tanya AI** beranda — pertanyaan langsung terkirim ke asisten → jawaban dengan kutipan yang bisa dibuka ke bagian dokumen. Lanjutkan di utas yang sama: "Jelaskan lebih lengkap" → uraian langkah konfigurasi dari sumber yang sama; "Kalau perangkat authenticator-nya hilang?" → "tidak ada bagian dokumen yang menjawab" (korpus memang tidak membahasnya, dan model tidak mengarang); "Dan kalau koneksinya gagal setelah itu?" → langkah persis dari bagian "Jika koneksi gagal" — pertanyaan lanjutan dipahami dari riwayat ([docs/WEKNORA.md §24](docs/WEKNORA.md)). "harga saham" → "tidak tahu" tanpa memanggil model; injeksi "abaikan aturan akses" → tetap "tidak tahu". Ubah **ruang lingkup** ke satu kategori atau ke "dokumen yang saya buka"; riwayat percakapan tersimpan hanya untuk pemiliknya. Nilai jawaban "Membantu / tidak".
