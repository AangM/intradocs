...
| **Pemindai & converter di container** | Profil staging/production memindai dan mengonversi lewat loopback (sama seperti lokal), jadi host tunggal dengan `pnpm knowledge:start` berfungsi. `compose.prod.yaml` belum menyertakan ClamAV dan converter; tanpa keduanya unggah **ditolak** (fail-closed), bukan dilewati. Menambahkannya perlu keputusan jaringan internal antarcontainer. |
