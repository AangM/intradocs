...
| Q5 UI/aksesibilitas   | Visual DONE (UI.md §5, U09–U12); **axe + keyboard-only LULUS** (`tests/e2e/a11y.spec.ts`, desktop + mobile); load test BELUM (butuh hardware target)                  | `pnpm ui:shots`, `pnpm exec playwright test tests/e2e/a11y.spec.ts`                                                                           |
| Q6 operasi            | SEBAGIAN — `ops:backup`, `ops:verify-backup` (restore drill ke DB uji), `ops:restore`, `ops:ready` ada dan dijalankan lokal; rollback rilis, alarm, D04/D05/D07 BELUM | `pnpm ops:backup` → `pnpm ops:verify-backup <folder>`: 9 tabel, 34 migrasi, 69 berkas cocok hash                                              |
...
review, UAT pemilik domain; (b) format V1 yang butuh OCR/memori atau desain tersendiri — PDF
pindai, DOC lama, ZIP (PPTX dan HTML sudah); (c) satu fitur produk — **editor role kustom**:
lima role bawaan tertanam di 45 ekspresi policy RLS (`app.actor_role()`), jadi role kustom
berarti mendesain ulang lapisan izin di database menjadi tabel kemampuan — pekerjaan keamanan
tersendiri, bukan tambahan UI; (d) yang butuh hardware/lingkungan nyata — load test Q5,
rollback rilis dan alarm Q6, D04/D05/D07. Tidak satu pun dari itu boleh disebut selesai lewat
stub.
