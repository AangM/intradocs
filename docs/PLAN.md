...
| S05   | ✅ empat langkah, multi-file, 50 MiB, konversi→preview→metadata→submit, MD/TXT/PDF berteks/DOCX/XLSX, duplicate warning                                                                                                                                                                                                           | ◐ bantuan metadata ✅; **PPTX ✅ via parser WeKnora** (WEKNORA.md §27, hanya bila WeKnora aktif); **OCR/HTML/DOC/ZIP ✗** — tidak ditampilkan sebagai didukung |
...
review, UAT pemilik domain; (b) format V1 yang butuh OCR/memori — PDF pindai, HTML, DOC lama,
ZIP (PPTX sudah, lewat parser WeKnora); (c) dua fitur produk kecil — role kustom, laporan
ekspor dashboard (konflik antar-sumber ada untuk angka, belum untuk kata-kata); (d) dua gate
