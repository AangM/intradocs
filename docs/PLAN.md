...
| Q6 operasi            | SEBAGIAN — `ops:backup`, `ops:verify-backup` (restore drill ke DB uji), `ops:restore`, `ops:ready` ada dan dijalankan lokal; rollback rilis, alarm, D04/D05/D07 BELUM | `pnpm ops:backup` → `pnpm ops:verify-backup <folder>`: 9 tabel, 40 migrasi, 69 berkas cocok hash                                              |
...
pindai, DOC lama, ZIP (PPTX dan HTML sudah); (c) — _(editor role kustom sudah ada: dibangun sebagai
penyempitan di atas role bawaan, jadi 41 policy RLS tidak disentuh; lihat UI.md U14)_; (d) yang
butuh hardware/lingkungan nyata — load test Q5,
