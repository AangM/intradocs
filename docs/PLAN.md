...
| Q6 operasi            | SEBAGIAN — `ops:backup`, `ops:verify-backup` (restore drill ke DB uji), `ops:restore`, `ops:ready` ada dan dijalankan lokal; rollback rilis, alarm, D04/D05/D07 BELUM | `pnpm ops:backup` → `pnpm ops:verify-backup <folder>`: 9 tabel, 37 migrasi, 69 berkas cocok hash                                              |
