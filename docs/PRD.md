...
Status v0.2: acuan development lokal, dua developer tanpa deadline. Keputusan lintas dokumen mengikuti [README.md](README.md). Kontrak teknis berada di [ARCHITECTURE.md](ARCHITECTURE.md), visual di [UI.md](UI.md), bukti penerimaan di [PLAN.md](PLAN.md).
...
**MVP lokal** = alur end-to-end yang bisa diuji dengan login lokal dan data sintetis; bukan klaim siap produksi. **Pilot/MVP internal Telkom** = alur produksi terbatas dengan SSO/kebijakan data dan kontrol keamanan lengkap, bukan sekadar demo UI. **V1** = pelengkapan fungsi mockup setelah pilot. **Lanjut** = ekspansi enterprise yang tidak diperlukan untuk memvalidasi produk awal. Penundaan di bawah harus disetujui lewat D03.
